...
上式为传递函数的零极点表达式，零点或极点是实数或共 轭复数。 $k$ 称为零极点增益或根轨迹增益。
$$
...
$$
传递函数为： $G(s)=\frac{Y(s)}{R(s)}=\frac K{Ts+1}$ 
...
y(t)=K\int r(t)dt
$$
传递函数为： $G(s)=\frac{Y(s)}{R(s)}=\frac Ks$ 
