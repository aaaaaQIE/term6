...
他们的单位都是焦耳，表示能量，但他们的呈现形式不同，所以不是相同性质的参数，但是可以相互转化。
...
不可以。因为功是过程量而非状态量，不能单纯用初末状态的值相减，而应采用 $\int_1^2 p\mathrm dv$ 来表示膨胀功。
